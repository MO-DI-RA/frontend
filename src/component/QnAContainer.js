        ...
        <div className="QnA_Main" style={{ marginBottom: "40px" }}>
          {QnAList.map((qna) => {
            ...
            return (
              <QnAComponent
                ...
                onClick={(e) => modifyList && clickContainer(qna.id, e)}>
                <div
                  key={qna.id}
                  className={`Main_QnA ${
                    modifyList && selectedQID.includes(qna.id) ? "selected" : ""
                  }`}>
                  ...
                  <div className="summary"> {qna.content} </div>
                  <div className="QnA_content">
                    <img
                      src={
                        "http://localhost:8000/" + qna.author_profile_image ||
                        defaultImg
                      }
                      alt="기본 이미지"
                      className="profile"
          ...
          {modify ? (
            <div style={{ textAlign: "center" }}>
              {modifyList ? (
                <button onClick={clickDelete} className="LikeModifyBtn">
                  {" "}
                  관심 해제 하기{" "}
                </button>
              ) : (
                <button onClick={clickModify} className="LikeModifyBtn">
                  {" "}
                  수정하기{" "}
                </button>
              )}
